...
if not DB_PATH.exists():
    st.error("Database not found. Run ingestion from the **Generate** page first.")
    st.stop()
...
if df is None or df.empty:
    if view_mode == "Processed newsletter items":
        st.error(
            "Processed newsletter table not found. Generate a newsletter first."
        )
    else:
        st.error("No data found. Run ingestion from the **Generate** page first.")
    st.stop()
